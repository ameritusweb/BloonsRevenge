 ...
 */
export function generateRandomPath(options = {}) {
  const {
    startX = -14,
    startZ = -14,
    endX = 14,
 ...
 */
export function generateTowers(scene, pathPoints, options = {}) {
  const {
    count = 5,
    minDistFromPath = 3,
  ...
  while (towers.length < count && attempts < maxAttempts) {
    ...
    const tower = new Tower(scene, position, towerType);
    towers.push(tower);
  }
 ...
 */
export function applyLevelTheme(scene, themeName, options = {}) {
  ...
  switch(themeName) {
    case 'desert':
      return applyDesertTheme(scene, groundMesh, groundColor, modifiers);
    case 'fog':
      return applyFogTheme(scene, groundMesh, groundColor, modifiers);
    case 'storm':
      return applyStormTheme(scene, groundMesh, groundColor, modifiers);
    default:
      return modifiers;
  }
}
 ...
 * @private
 */
function applyDesertTheme(scene, groundMesh, groundColor, baseModifiers) {
  // Set desert sky color
  scene.clearColor = new BABYLON.Color3(0.8, 0.7, 0.5);
  ...
  const dustSystem = new BABYLON.ParticleSystem("dust", 1000, scene);
...
function applyFogTheme(scene, groundMesh, groundColor, baseModifiers) {
  ...
  const fogSystem = new BABYLON.ParticleSystem("fog", 5000, scene);
 ...
 * @private
 */
function applyStormTheme(scene, groundMesh, groundColor, baseModifiers) {
  // Set storm sky color
  ...
  let lastFlash = 0;
  const originalLightIntensity = scene.lights[0]?.intensity || 1.0;
 ...
 */
export function selectRandomTheme(level) {
 ...
 */
export function createLevel(scene, options = {}) {
  const {
    level = 1,
    theme = 'random'
  } = options;
  ...
  // Determine theme
  const actualTheme = theme === 'random' ? selectRandomTheme(level) : theme;
  
  // Generate path
  const pathComplexity = Math.min(5 + level * 0.3, 10); // Increase complexity with level
  const pathPoints = generateRandomPath({ complexity: pathComplexity });
  
  // Create path mesh
  const pathMesh = createPathMesh(scene, pathPoints);
  ...
  const towers = generateTowers(scene, pathPoints, {
    count: towerCount,
    level: level
  });
  
  // Apply theme
  const themeModifiers = applyLevelTheme(scene, actualTheme, { groundMesh: ground });
